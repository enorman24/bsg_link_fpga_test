...
//
// PHY: both synthesis and simulation use the UltraScale+ hard PHY
//   (basejump_stl/hard/ultrascale_plus/bsg_link/), which instantiates ODDRE1 /
//   IDDRE1 / IDELAYE3. bsg_link_ddr_upstream forwards the link clock from
//   io_clk90_i, a 90°-shifted copy of io_clk_i.
//     Synthesis  : io_clk90_i comes from clk_wiz_0 CLKOUT2 (core_clk90_w).
//     Simulation (+define+SIM): the testbench drives core_clk90_i (a quarter-period
//       lagging copy of core_clk_i); xsim simulates the hard primitives via unisims.
//       The IDDRE1+FIXED-IDELAYE3 capture is 90°-centered, so the sim clock is run
//       slow enough that the fixed IDELAY stays well under a quarter period.
//
// Vivado project setup:
//   - Run scripts/create_project.tcl to generate the jtag_axi_0 and axi_switch_0 IP.
//
// Simulation:
...
#(
  ...
  // =========================================================================
  // Link 2 — independent reverse-direction bsg_link on the other F2G headers.
  //   TX2 (upstream2) = FMC_HPC1 / F2G GPIO_0
  //   RX2 (downstream2) = FMC_HPC0 / F2G GPIO_1
  // Same structure as link 1; separate pins/banks and AXI address space.
  // =========================================================================
  input  logic [NUM_CHANNELS_P-1:0]                      token_clk2_i,

  // ---- Link 2 TX outputs → ribbon cable ----
  output logic [NUM_CHANNELS_P-1:0]                      upstream2_io_clk_r_o,
  output logic [NUM_CHANNELS_P-1:0][CHANNEL_WIDTH_P-1:0] upstream2_io_data_r_o,
  output logic [NUM_CHANNELS_P-1:0]                      upstream2_io_valid_r_o,

  // ---- Link 2 RX inputs ← ribbon cable ----
  input  logic [NUM_CHANNELS_P-1:0]                      downstream2_io_clk_i,
  input  logic [NUM_CHANNELS_P-1:0][CHANNEL_WIDTH_P-1:0] downstream2_io_data_i,
  input  logic [NUM_CHANNELS_P-1:0]                      downstream2_io_valid_i,
  output logic [NUM_CHANNELS_P-1:0]                      downstream2_core_token_r_o
`ifdef SIM
  // Simulation-only ports — absent in synthesis.
  // core_clk_i:   testbench-driven clock replacing clk_wiz_0 clk_out1.
  // core_clk90_i: testbench-driven 90°-shifted clock replacing clk_wiz_0 clk_out2.
  //               The hard ODDR PHY forwards the link clock from this, so sim must
  //               drive it (in synthesis it comes from clk_wiz_0; absent here).
...
`endif
  ...
  logic core_clk90_w;   // 90° clock for the forwarded-clock ODDRE1 (both upstreams)
`ifndef SIM
  clk_wiz_0 i_clk_wiz_0 (
    .clk_in1_p (clk_in1_p),
    .clk_in1_n (clk_in1_n),
    .clk_out1  (core_clk_w),
    .clk_out2  (core_clk90_w),
    .locked    (mmcm_locked_w)
  );
`else
  assign core_clk_w    = core_clk_i;
  assign core_clk90_w  = core_clk90_i;
  assign mmcm_locked_w = 1'b1;
`endif

`ifndef SIM
  vio_0 i_vio_0 (
    .clk        (core_clk_w),
    .probe_out0 (vio_rst_w)
  );
`endif
  ...
  always_ff @(posedge core_clk_w) begin
    if (rst_int_w) reset_cnt_r <= '0;
    else if (reset_cnt_r < 6'd63) reset_cnt_r <= reset_cnt_r + 6'd1;
  end

  logic async_token_reset_int;
  assign async_token_reset_int = !rst_int_w && (reset_cnt_r >= 6'd2) && (reset_cnt_r < 6'd5);

  // Upstream io reset: deasserts at count 16.
  logic io_link_reset_int;
  assign io_link_reset_int = rst_int_w || (reset_cnt_r < 6'd16);
  ...
  assign downstream_io_link_reset_int = rst_int_w || (reset_cnt_r < 6'd32);

  logic core_link_reset_int;
  assign core_link_reset_int = rst_int_w || (reset_cnt_r < 6'd32);
...
`endif
  ...
  // =========================================================================
  // AXI switch: 1 slave port (JTAG → S00), 3 master ports
  // (TX FIFO → M00, RX FIFO → M01, RX Status → M02).
  // Vivado AXI Switch IP (axi_switch_0); address map configured at IP-gen time.
  // =========================================================================
  ...
  // =========================================================================
  // Link 2 crossbar master ports: M03 TX2 FIFO, M04 RX2 FIFO, M05 RX2 Status.
  // These mirror M00/M01/M02 exactly (same packing) on the link-2 address map.
  // =========================================================================
  mst_req_t  tx2_axi_req_w,  rx2_axi_req_w,  sta2_axi_req_w;
  mst_resp_t tx2_axi_resp_w, rx2_axi_resp_w, sta2_axi_resp_w;

  // ---- M03 wires: TX2 FIFO (0x4003_0000 – 0x4003_FFFF) ----
  logic        xbar_m03_aw_valid_w;  logic [0:0]  xbar_m03_aw_id_w;
  logic [31:0] xbar_m03_aw_addr_w;  logic [7:0]  xbar_m03_aw_len_w;
  logic [2:0]  xbar_m03_aw_size_w;  logic [1:0]  xbar_m03_aw_burst_w;
  logic [0:0]  xbar_m03_aw_lock_w;  logic [3:0]  xbar_m03_aw_cache_w;
  logic [2:0]  xbar_m03_aw_prot_w;  logic [3:0]  xbar_m03_aw_qos_w;
  logic [0:0]  xbar_m03_aw_user_w;
  logic        xbar_m03_w_valid_w;   logic [31:0] xbar_m03_w_data_w;
  logic [3:0]  xbar_m03_w_strb_w;   logic        xbar_m03_w_last_w;
  logic [0:0]  xbar_m03_w_user_w;   logic        xbar_m03_b_ready_w;
  logic        xbar_m03_ar_valid_w;  logic [0:0]  xbar_m03_ar_id_w;
  logic [31:0] xbar_m03_ar_addr_w;  logic [7:0]  xbar_m03_ar_len_w;
  logic [2:0]  xbar_m03_ar_size_w;  logic [1:0]  xbar_m03_ar_burst_w;
  logic [0:0]  xbar_m03_ar_lock_w;  logic [3:0]  xbar_m03_ar_cache_w;
  logic [2:0]  xbar_m03_ar_prot_w;  logic [3:0]  xbar_m03_ar_qos_w;
  logic [0:0]  xbar_m03_ar_user_w;  logic        xbar_m03_r_ready_w;
  logic        xbar_m03_aw_ready_w;  logic        xbar_m03_w_ready_w;
  logic        xbar_m03_b_valid_w;   logic [0:0]  xbar_m03_b_id_w;
  logic [1:0]  xbar_m03_b_resp_w;   logic [0:0]  xbar_m03_b_user_w;
  logic        xbar_m03_ar_ready_w;  logic        xbar_m03_r_valid_w;
  logic [0:0]  xbar_m03_r_id_w;     logic [31:0] xbar_m03_r_data_w;
  logic [1:0]  xbar_m03_r_resp_w;   logic        xbar_m03_r_last_w;
  logic [0:0]  xbar_m03_r_user_w;

  always_comb begin : pack_m03_req
    tx2_axi_req_w          = '0;
    tx2_axi_req_w.aw_valid = xbar_m03_aw_valid_w;
    tx2_axi_req_w.aw.id    = xbar_m03_aw_id_w;
    tx2_axi_req_w.aw.addr  = xbar_m03_aw_addr_w;
    tx2_axi_req_w.aw.len   = xbar_m03_aw_len_w;
    tx2_axi_req_w.aw.size  = xbar_m03_aw_size_w;
    tx2_axi_req_w.aw.burst = xbar_m03_aw_burst_w;
    tx2_axi_req_w.aw.lock  = xbar_m03_aw_lock_w;
    tx2_axi_req_w.aw.cache = xbar_m03_aw_cache_w;
    tx2_axi_req_w.aw.prot  = xbar_m03_aw_prot_w;
    tx2_axi_req_w.aw.qos   = xbar_m03_aw_qos_w;
    tx2_axi_req_w.aw.user  = xbar_m03_aw_user_w;
    tx2_axi_req_w.w_valid  = xbar_m03_w_valid_w;
    tx2_axi_req_w.w.data   = xbar_m03_w_data_w;
    tx2_axi_req_w.w.strb   = xbar_m03_w_strb_w;
    tx2_axi_req_w.w.last   = xbar_m03_w_last_w;
    tx2_axi_req_w.w.user   = xbar_m03_w_user_w;
    tx2_axi_req_w.b_ready  = xbar_m03_b_ready_w;
    tx2_axi_req_w.ar_valid = xbar_m03_ar_valid_w;
    tx2_axi_req_w.ar.id    = xbar_m03_ar_id_w;
    tx2_axi_req_w.ar.addr  = xbar_m03_ar_addr_w;
    tx2_axi_req_w.ar.len   = xbar_m03_ar_len_w;
    tx2_axi_req_w.ar.size  = xbar_m03_ar_size_w;
    tx2_axi_req_w.ar.burst = xbar_m03_ar_burst_w;
    tx2_axi_req_w.ar.lock  = xbar_m03_ar_lock_w;
    tx2_axi_req_w.ar.cache = xbar_m03_ar_cache_w;
    tx2_axi_req_w.ar.prot  = xbar_m03_ar_prot_w;
    tx2_axi_req_w.ar.qos   = xbar_m03_ar_qos_w;
    tx2_axi_req_w.ar.user  = xbar_m03_ar_user_w;
    tx2_axi_req_w.r_ready  = xbar_m03_r_ready_w;
  end

  assign xbar_m03_aw_ready_w = tx2_axi_resp_w.aw_ready;
  assign xbar_m03_w_ready_w  = tx2_axi_resp_w.w_ready;
  assign xbar_m03_b_valid_w  = tx2_axi_resp_w.b_valid;
  assign xbar_m03_b_id_w     = tx2_axi_resp_w.b.id;
  assign xbar_m03_b_resp_w   = tx2_axi_resp_w.b.resp;
  assign xbar_m03_b_user_w   = tx2_axi_resp_w.b.user;
  assign xbar_m03_ar_ready_w = tx2_axi_resp_w.ar_ready;
  assign xbar_m03_r_valid_w  = tx2_axi_resp_w.r_valid;
  assign xbar_m03_r_id_w     = tx2_axi_resp_w.r.id;
  assign xbar_m03_r_data_w   = tx2_axi_resp_w.r.data;
  assign xbar_m03_r_resp_w   = tx2_axi_resp_w.r.resp;
  assign xbar_m03_r_last_w   = tx2_axi_resp_w.r.last;
  assign xbar_m03_r_user_w   = tx2_axi_resp_w.r.user;

  // ---- M04 wires: RX2 FIFO (0x4004_0000 – 0x4004_FFFF) ----
  logic        xbar_m04_aw_valid_w;  logic [0:0]  xbar_m04_aw_id_w;
  logic [31:0] xbar_m04_aw_addr_w;  logic [7:0]  xbar_m04_aw_len_w;
  logic [2:0]  xbar_m04_aw_size_w;  logic [1:0]  xbar_m04_aw_burst_w;
  logic [0:0]  xbar_m04_aw_lock_w;  logic [3:0]  xbar_m04_aw_cache_w;
  logic [2:0]  xbar_m04_aw_prot_w;  logic [3:0]  xbar_m04_aw_qos_w;
  logic [0:0]  xbar_m04_aw_user_w;
  logic        xbar_m04_w_valid_w;   logic [31:0] xbar_m04_w_data_w;
  logic [3:0]  xbar_m04_w_strb_w;   logic        xbar_m04_w_last_w;
  logic [0:0]  xbar_m04_w_user_w;   logic        xbar_m04_b_ready_w;
  logic        xbar_m04_ar_valid_w;  logic [0:0]  xbar_m04_ar_id_w;
  logic [31:0] xbar_m04_ar_addr_w;  logic [7:0]  xbar_m04_ar_len_w;
  logic [2:0]  xbar_m04_ar_size_w;  logic [1:0]  xbar_m04_ar_burst_w;
  logic [0:0]  xbar_m04_ar_lock_w;  logic [3:0]  xbar_m04_ar_cache_w;
  logic [2:0]  xbar_m04_ar_prot_w;  logic [3:0]  xbar_m04_ar_qos_w;
  logic [0:0]  xbar_m04_ar_user_w;  logic        xbar_m04_r_ready_w;
  logic        xbar_m04_aw_ready_w;  logic        xbar_m04_w_ready_w;
  logic        xbar_m04_b_valid_w;   logic [0:0]  xbar_m04_b_id_w;
  logic [1:0]  xbar_m04_b_resp_w;   logic [0:0]  xbar_m04_b_user_w;
  logic        xbar_m04_ar_ready_w;  logic        xbar_m04_r_valid_w;
  logic [0:0]  xbar_m04_r_id_w;     logic [31:0] xbar_m04_r_data_w;
  logic [1:0]  xbar_m04_r_resp_w;   logic        xbar_m04_r_last_w;
  logic [0:0]  xbar_m04_r_user_w;

  always_comb begin : pack_m04_req
    rx2_axi_req_w          = '0;
    rx2_axi_req_w.aw_valid = xbar_m04_aw_valid_w;
    rx2_axi_req_w.aw.id    = xbar_m04_aw_id_w;
    rx2_axi_req_w.aw.addr  = xbar_m04_aw_addr_w;
    rx2_axi_req_w.aw.len   = xbar_m04_aw_len_w;
    rx2_axi_req_w.aw.size  = xbar_m04_aw_size_w;
    rx2_axi_req_w.aw.burst = xbar_m04_aw_burst_w;
    rx2_axi_req_w.aw.lock  = xbar_m04_aw_lock_w;
    rx2_axi_req_w.aw.cache = xbar_m04_aw_cache_w;
    rx2_axi_req_w.aw.prot  = xbar_m04_aw_prot_w;
    rx2_axi_req_w.aw.qos   = xbar_m04_aw_qos_w;
    rx2_axi_req_w.aw.user  = xbar_m04_aw_user_w;
    rx2_axi_req_w.w_valid  = xbar_m04_w_valid_w;
    rx2_axi_req_w.w.data   = xbar_m04_w_data_w;
    rx2_axi_req_w.w.strb   = xbar_m04_w_strb_w;
    rx2_axi_req_w.w.last   = xbar_m04_w_last_w;
    rx2_axi_req_w.w.user   = xbar_m04_w_user_w;
    rx2_axi_req_w.b_ready  = xbar_m04_b_ready_w;
    rx2_axi_req_w.ar_valid = xbar_m04_ar_valid_w;
    rx2_axi_req_w.ar.id    = xbar_m04_ar_id_w;
    rx2_axi_req_w.ar.addr  = xbar_m04_ar_addr_w;
    rx2_axi_req_w.ar.len   = xbar_m04_ar_len_w;
    rx2_axi_req_w.ar.size  = xbar_m04_ar_size_w;
    rx2_axi_req_w.ar.burst = xbar_m04_ar_burst_w;
    rx2_axi_req_w.ar.lock  = xbar_m04_ar_lock_w;
    rx2_axi_req_w.ar.cache = xbar_m04_ar_cache_w;
    rx2_axi_req_w.ar.prot  = xbar_m04_ar_prot_w;
    rx2_axi_req_w.ar.qos   = xbar_m04_ar_qos_w;
    rx2_axi_req_w.ar.user  = xbar_m04_ar_user_w;
    rx2_axi_req_w.r_ready  = xbar_m04_r_ready_w;
  end

  assign xbar_m04_aw_ready_w = rx2_axi_resp_w.aw_ready;
  assign xbar_m04_w_ready_w  = rx2_axi_resp_w.w_ready;
  assign xbar_m04_b_valid_w  = rx2_axi_resp_w.b_valid;
  assign xbar_m04_b_id_w     = rx2_axi_resp_w.b.id;
  assign xbar_m04_b_resp_w   = rx2_axi_resp_w.b.resp;
  assign xbar_m04_b_user_w   = rx2_axi_resp_w.b.user;
  assign xbar_m04_ar_ready_w = rx2_axi_resp_w.ar_ready;
  assign xbar_m04_r_valid_w  = rx2_axi_resp_w.r_valid;
  assign xbar_m04_r_id_w     = rx2_axi_resp_w.r.id;
  assign xbar_m04_r_data_w   = rx2_axi_resp_w.r.data;
  assign xbar_m04_r_resp_w   = rx2_axi_resp_w.r.resp;
  assign xbar_m04_r_last_w   = rx2_axi_resp_w.r.last;
  assign xbar_m04_r_user_w   = rx2_axi_resp_w.r.user;

  // ---- M05 wires: RX2 Status (0x4005_0000 – 0x4005_FFFF) ----
  logic        xbar_m05_aw_valid_w;  logic [0:0]  xbar_m05_aw_id_w;
  logic [31:0] xbar_m05_aw_addr_w;  logic [7:0]  xbar_m05_aw_len_w;
  logic [2:0]  xbar_m05_aw_size_w;  logic [1:0]  xbar_m05_aw_burst_w;
  logic [0:0]  xbar_m05_aw_lock_w;  logic [3:0]  xbar_m05_aw_cache_w;
  logic [2:0]  xbar_m05_aw_prot_w;  logic [3:0]  xbar_m05_aw_qos_w;
  logic [0:0]  xbar_m05_aw_user_w;
  logic        xbar_m05_w_valid_w;   logic [31:0] xbar_m05_w_data_w;
  logic [3:0]  xbar_m05_w_strb_w;   logic        xbar_m05_w_last_w;
  logic [0:0]  xbar_m05_w_user_w;   logic        xbar_m05_b_ready_w;
  logic        xbar_m05_ar_valid_w;  logic [0:0]  xbar_m05_ar_id_w;
  logic [31:0] xbar_m05_ar_addr_w;  logic [7:0]  xbar_m05_ar_len_w;
  logic [2:0]  xbar_m05_ar_size_w;  logic [1:0]  xbar_m05_ar_burst_w;
  logic [0:0]  xbar_m05_ar_lock_w;  logic [3:0]  xbar_m05_ar_cache_w;
  logic [2:0]  xbar_m05_ar_prot_w;  logic [3:0]  xbar_m05_ar_qos_w;
  logic [0:0]  xbar_m05_ar_user_w;  logic        xbar_m05_r_ready_w;
  logic        xbar_m05_aw_ready_w;  logic        xbar_m05_w_ready_w;
  logic        xbar_m05_b_valid_w;   logic [0:0]  xbar_m05_b_id_w;
  logic [1:0]  xbar_m05_b_resp_w;   logic [0:0]  xbar_m05_b_user_w;
  logic        xbar_m05_ar_ready_w;  logic        xbar_m05_r_valid_w;
  logic [0:0]  xbar_m05_r_id_w;     logic [31:0] xbar_m05_r_data_w;
  logic [1:0]  xbar_m05_r_resp_w;   logic        xbar_m05_r_last_w;
  logic [0:0]  xbar_m05_r_user_w;

  always_comb begin : pack_m05_req
    sta2_axi_req_w          = '0;
    sta2_axi_req_w.aw_valid = xbar_m05_aw_valid_w;
    sta2_axi_req_w.aw.id    = xbar_m05_aw_id_w;
    sta2_axi_req_w.aw.addr  = xbar_m05_aw_addr_w;
    sta2_axi_req_w.aw.len   = xbar_m05_aw_len_w;
    sta2_axi_req_w.aw.size  = xbar_m05_aw_size_w;
    sta2_axi_req_w.aw.burst = xbar_m05_aw_burst_w;
    sta2_axi_req_w.aw.lock  = xbar_m05_aw_lock_w;
    sta2_axi_req_w.aw.cache = xbar_m05_aw_cache_w;
    sta2_axi_req_w.aw.prot  = xbar_m05_aw_prot_w;
    sta2_axi_req_w.aw.qos   = xbar_m05_aw_qos_w;
    sta2_axi_req_w.aw.user  = xbar_m05_aw_user_w;
    sta2_axi_req_w.w_valid  = xbar_m05_w_valid_w;
    sta2_axi_req_w.w.data   = xbar_m05_w_data_w;
    sta2_axi_req_w.w.strb   = xbar_m05_w_strb_w;
    sta2_axi_req_w.w.last   = xbar_m05_w_last_w;
    sta2_axi_req_w.w.user   = xbar_m05_w_user_w;
    sta2_axi_req_w.b_ready  = xbar_m05_b_ready_w;
    sta2_axi_req_w.ar_valid = xbar_m05_ar_valid_w;
    sta2_axi_req_w.ar.id    = xbar_m05_ar_id_w;
    sta2_axi_req_w.ar.addr  = xbar_m05_ar_addr_w;
    sta2_axi_req_w.ar.len   = xbar_m05_ar_len_w;
    sta2_axi_req_w.ar.size  = xbar_m05_ar_size_w;
    sta2_axi_req_w.ar.burst = xbar_m05_ar_burst_w;
    sta2_axi_req_w.ar.lock  = xbar_m05_ar_lock_w;
    sta2_axi_req_w.ar.cache = xbar_m05_ar_cache_w;
    sta2_axi_req_w.ar.prot  = xbar_m05_ar_prot_w;
    sta2_axi_req_w.ar.qos   = xbar_m05_ar_qos_w;
    sta2_axi_req_w.ar.user  = xbar_m05_ar_user_w;
    sta2_axi_req_w.r_ready  = xbar_m05_r_ready_w;
  end

  assign xbar_m05_aw_ready_w = sta2_axi_resp_w.aw_ready;
  assign xbar_m05_w_ready_w  = sta2_axi_resp_w.w_ready;
  assign xbar_m05_b_valid_w  = sta2_axi_resp_w.b_valid;
  assign xbar_m05_b_id_w     = sta2_axi_resp_w.b.id;
  assign xbar_m05_b_resp_w   = sta2_axi_resp_w.b.resp;
  assign xbar_m05_b_user_w   = sta2_axi_resp_w.b.user;
  assign xbar_m05_ar_ready_w = sta2_axi_resp_w.ar_ready;
  assign xbar_m05_r_valid_w  = sta2_axi_resp_w.r_valid;
  assign xbar_m05_r_id_w     = sta2_axi_resp_w.r.id;
  assign xbar_m05_r_data_w   = sta2_axi_resp_w.r.data;
  assign xbar_m05_r_resp_w   = sta2_axi_resp_w.r.resp;
  assign xbar_m05_r_last_w   = sta2_axi_resp_w.r.last;
  assign xbar_m05_r_user_w   = sta2_axi_resp_w.r.user;
  ...
  axi_switch_0 i_xbar (
    ...
    // M02: RX Status (AXI4-Lite — no id/len/size/burst/lock/cache/qos/user/last ports)
    ...
    // M03: TX2 FIFO
    .m03_axi_awid    (xbar_m03_aw_id_w),
    .m03_axi_awaddr  (xbar_m03_aw_addr_w),
    .m03_axi_awlen   (xbar_m03_aw_len_w),
    .m03_axi_awsize  (xbar_m03_aw_size_w),
    .m03_axi_awburst (xbar_m03_aw_burst_w),
    .m03_axi_awlock  (xbar_m03_aw_lock_w),
    .m03_axi_awcache (xbar_m03_aw_cache_w),
    .m03_axi_awprot  (xbar_m03_aw_prot_w),
    .m03_axi_awqos   (xbar_m03_aw_qos_w),
    .m03_axi_awuser  (xbar_m03_aw_user_w),
    .m03_axi_awvalid (xbar_m03_aw_valid_w),
    .m03_axi_awready (xbar_m03_aw_ready_w),
    .m03_axi_wdata   (xbar_m03_w_data_w),
    .m03_axi_wstrb   (xbar_m03_w_strb_w),
    .m03_axi_wlast   (xbar_m03_w_last_w),
    .m03_axi_wuser   (xbar_m03_w_user_w),
    .m03_axi_wvalid  (xbar_m03_w_valid_w),
    .m03_axi_wready  (xbar_m03_w_ready_w),
    .m03_axi_bid     (xbar_m03_b_id_w),
    .m03_axi_bresp   (xbar_m03_b_resp_w),
    .m03_axi_buser   (xbar_m03_b_user_w),
    .m03_axi_bvalid  (xbar_m03_b_valid_w),
    .m03_axi_bready  (xbar_m03_b_ready_w),
    .m03_axi_arid    (xbar_m03_ar_id_w),
    .m03_axi_araddr  (xbar_m03_ar_addr_w),
    .m03_axi_arlen   (xbar_m03_ar_len_w),
    .m03_axi_arsize  (xbar_m03_ar_size_w),
    .m03_axi_arburst (xbar_m03_ar_burst_w),
    .m03_axi_arlock  (xbar_m03_ar_lock_w),
    .m03_axi_arcache (xbar_m03_ar_cache_w),
    .m03_axi_arprot  (xbar_m03_ar_prot_w),
    .m03_axi_arqos   (xbar_m03_ar_qos_w),
    .m03_axi_aruser  (xbar_m03_ar_user_w),
    .m03_axi_arvalid (xbar_m03_ar_valid_w),
    .m03_axi_arready (xbar_m03_ar_ready_w),
    .m03_axi_rid     (xbar_m03_r_id_w),
    .m03_axi_rdata   (xbar_m03_r_data_w),
    .m03_axi_rresp   (xbar_m03_r_resp_w),
    .m03_axi_rlast   (xbar_m03_r_last_w),
    .m03_axi_ruser   (xbar_m03_r_user_w),
    .m03_axi_rvalid  (xbar_m03_r_valid_w),
    .m03_axi_rready  (xbar_m03_r_ready_w),
    // M04: RX2 FIFO
    .m04_axi_awid    (xbar_m04_aw_id_w),
    .m04_axi_awaddr  (xbar_m04_aw_addr_w),
    .m04_axi_awlen   (xbar_m04_aw_len_w),
    .m04_axi_awsize  (xbar_m04_aw_size_w),
    .m04_axi_awburst (xbar_m04_aw_burst_w),
    .m04_axi_awlock  (xbar_m04_aw_lock_w),
    .m04_axi_awcache (xbar_m04_aw_cache_w),
    .m04_axi_awprot  (xbar_m04_aw_prot_w),
    .m04_axi_awqos   (xbar_m04_aw_qos_w),
    .m04_axi_awuser  (xbar_m04_aw_user_w),
    .m04_axi_awvalid (xbar_m04_aw_valid_w),
    .m04_axi_awready (xbar_m04_aw_ready_w),
    .m04_axi_wdata   (xbar_m04_w_data_w),
    .m04_axi_wstrb   (xbar_m04_w_strb_w),
    .m04_axi_wlast   (xbar_m04_w_last_w),
    .m04_axi_wuser   (xbar_m04_w_user_w),
    .m04_axi_wvalid  (xbar_m04_w_valid_w),
    .m04_axi_wready  (xbar_m04_w_ready_w),
    .m04_axi_bid     (xbar_m04_b_id_w),
    .m04_axi_bresp   (xbar_m04_b_resp_w),
    .m04_axi_buser   (xbar_m04_b_user_w),
    .m04_axi_bvalid  (xbar_m04_b_valid_w),
    .m04_axi_bready  (xbar_m04_b_ready_w),
    .m04_axi_arid    (xbar_m04_ar_id_w),
    .m04_axi_araddr  (xbar_m04_ar_addr_w),
    .m04_axi_arlen   (xbar_m04_ar_len_w),
    .m04_axi_arsize  (xbar_m04_ar_size_w),
    .m04_axi_arburst (xbar_m04_ar_burst_w),
    .m04_axi_arlock  (xbar_m04_ar_lock_w),
    .m04_axi_arcache (xbar_m04_ar_cache_w),
    .m04_axi_arprot  (xbar_m04_ar_prot_w),
    .m04_axi_arqos   (xbar_m04_ar_qos_w),
    .m04_axi_aruser  (xbar_m04_ar_user_w),
    .m04_axi_arvalid (xbar_m04_ar_valid_w),
    .m04_axi_arready (xbar_m04_ar_ready_w),
    .m04_axi_rid     (xbar_m04_r_id_w),
    .m04_axi_rdata   (xbar_m04_r_data_w),
    .m04_axi_rresp   (xbar_m04_r_resp_w),
    .m04_axi_rlast   (xbar_m04_r_last_w),
    .m04_axi_ruser   (xbar_m04_r_user_w),
    .m04_axi_rvalid  (xbar_m04_r_valid_w),
    .m04_axi_rready  (xbar_m04_r_ready_w),
    // M05: RX2 Status (AXI4-Lite — no id/len/size/burst/lock/cache/qos/user/last ports)
    .m05_axi_awaddr  (xbar_m05_aw_addr_w),
    .m05_axi_awprot  (xbar_m05_aw_prot_w),
    .m05_axi_awvalid (xbar_m05_aw_valid_w),
    .m05_axi_awready (xbar_m05_aw_ready_w),
    .m05_axi_wdata   (xbar_m05_w_data_w),
    .m05_axi_wstrb   (xbar_m05_w_strb_w),
    .m05_axi_wvalid  (xbar_m05_w_valid_w),
    .m05_axi_wready  (xbar_m05_w_ready_w),
    .m05_axi_bresp   (xbar_m05_b_resp_w),
    .m05_axi_bvalid  (xbar_m05_b_valid_w),
    .m05_axi_bready  (xbar_m05_b_ready_w),
    .m05_axi_araddr  (xbar_m05_ar_addr_w),
    .m05_axi_arprot  (xbar_m05_ar_prot_w),
    .m05_axi_arvalid (xbar_m05_ar_valid_w),
    .m05_axi_arready (xbar_m05_ar_ready_w),
    .m05_axi_rdata   (xbar_m05_r_data_w),
    .m05_axi_rresp   (xbar_m05_r_resp_w),
    .m05_axi_rvalid  (xbar_m05_r_valid_w),
    .m05_axi_rready  (xbar_m05_r_ready_w)
  );

`ifndef SIM
  // =========================================================================
  // ILA debug probes — excluded from simulation (no sim model in Vivado 2025.1)
  // =========================================================================

  // ILA 1: JTAG AXI master ↔ AXI switch (S00)
  ila_jtag_sw i_ila_jtag_sw (
    .clk    (core_clk_w),
    .probe0 ({jtag_req_w.aw_valid,    xbar_s00_aw_ready_w,
              jtag_req_w.w_valid,     xbar_s00_w_ready_w,
              xbar_s00_b_valid_w,     jtag_req_w.b_ready,
              jtag_req_w.ar_valid,    xbar_s00_ar_ready_w,
              xbar_s00_r_valid_w,     jtag_req_w.r_ready,
              xbar_s00_r_last_w,      jtag_req_w.w.last}),
    .probe1 (jtag_req_w.aw.addr),
    .probe2 (jtag_req_w.w.data),
    .probe3 (jtag_req_w.ar.addr),
    .probe4 (xbar_s00_r_data_w),
    .probe5 ({xbar_s00_b_resp_w, xbar_s00_r_resp_w})
  );

  // ILA 2: AXI switch M00 ↔ TX FIFO
  ila_sw_tx i_ila_sw_tx (
    .clk    (core_clk_w),
    .probe0 ({xbar_m00_aw_valid_w, xbar_m00_aw_ready_w,
              xbar_m00_w_valid_w,  xbar_m00_w_ready_w,
              xbar_m00_b_valid_w,  xbar_m00_b_ready_w,
              xbar_m00_ar_valid_w, xbar_m00_ar_ready_w,
              xbar_m00_r_valid_w,  xbar_m00_r_ready_w,
              xbar_m00_r_last_w,   xbar_m00_w_last_w}),
    .probe1 (xbar_m00_aw_addr_w),
    .probe2 (xbar_m00_w_data_w),
    .probe3 (xbar_m00_ar_addr_w),
    .probe4 (xbar_m00_r_data_w),
    .probe5 ({xbar_m00_b_resp_w, xbar_m00_r_resp_w})
  );

  // ILA 3: AXI switch M01 ↔ RX FIFO
  ila_sw_rx i_ila_sw_rx (
    .clk    (core_clk_w),
    .probe0 ({xbar_m01_aw_valid_w, xbar_m01_aw_ready_w,
              xbar_m01_w_valid_w,  xbar_m01_w_ready_w,
              xbar_m01_b_valid_w,  xbar_m01_b_ready_w,
              xbar_m01_ar_valid_w, xbar_m01_ar_ready_w,
              xbar_m01_r_valid_w,  xbar_m01_r_ready_w,
              xbar_m01_r_last_w,   xbar_m01_w_last_w}),
    .probe1 (xbar_m01_aw_addr_w),
    .probe2 (xbar_m01_w_data_w),
    .probe3 (xbar_m01_ar_addr_w),
    .probe4 (xbar_m01_r_data_w),
    .probe5 ({xbar_m01_b_resp_w, xbar_m01_r_resp_w})
  );

  // ILA 4: AXI switch M02 ↔ RX status register map (AXI4-Lite; no last signals)
  ila_sw_sta i_ila_sw_sta (
    .clk    (core_clk_w),
    .probe0 ({xbar_m02_aw_valid_w, xbar_m02_aw_ready_w,
              xbar_m02_w_valid_w,  xbar_m02_w_ready_w,
              xbar_m02_b_valid_w,  xbar_m02_b_ready_w,
              xbar_m02_ar_valid_w, xbar_m02_ar_ready_w,
              xbar_m02_r_valid_w,  xbar_m02_r_ready_w,
              1'b0,                1'b0}),
    .probe1 (xbar_m02_aw_addr_w),
    .probe2 (xbar_m02_w_data_w),
    .probe3 (xbar_m02_ar_addr_w),
    .probe4 (xbar_m02_r_data_w),
    .probe5 ({xbar_m02_b_resp_w, xbar_m02_r_resp_w})
  );

  // ILA 5: TX FIFO ↔ BSG link TX core
  ila_tx_bsg i_ila_tx_bsg (
    .clk    (core_clk_w),
    .probe0 ({tx_valid_w, tx_ready_w}),
    .probe1 (tx_data_w)
  );

  // ILA 6: RX FIFO ↔ BSG link RX core
  ila_rx_bsg i_ila_rx_bsg (
    .clk    (core_clk_w),
    .probe0 ({rx_valid_w, rx_yumi_w}),
    .probe1 (rx_data_w)
  );

  // ILA 7: RX FIFO status signals ↔ RX status register map
  ila_rx_sta i_ila_rx_sta (
    .clk    (core_clk_w),
    .probe0 ({rx_fifo_empty_w, rx_fifo_full_w, rx_fifo_count_w})
  );

  // ===== Link 2 ILAs (reverse-direction link; mirror ILAs 2-7) =====

  // ILA 8: AXI switch M03 ↔ TX2 FIFO
  ila_sw_tx2 i_ila_sw_tx2 (
    .clk    (core_clk_w),
    .probe0 ({xbar_m03_aw_valid_w, xbar_m03_aw_ready_w,
              xbar_m03_w_valid_w,  xbar_m03_w_ready_w,
              xbar_m03_b_valid_w,  xbar_m03_b_ready_w,
              xbar_m03_ar_valid_w, xbar_m03_ar_ready_w,
              xbar_m03_r_valid_w,  xbar_m03_r_ready_w,
              xbar_m03_r_last_w,   xbar_m03_w_last_w}),
    .probe1 (xbar_m03_aw_addr_w),
    .probe2 (xbar_m03_w_data_w),
    .probe3 (xbar_m03_ar_addr_w),
    .probe4 (xbar_m03_r_data_w),
    .probe5 ({xbar_m03_b_resp_w, xbar_m03_r_resp_w})
  );

  // ILA 9: AXI switch M04 ↔ RX2 FIFO
  ila_sw_rx2 i_ila_sw_rx2 (
    .clk    (core_clk_w),
    .probe0 ({xbar_m04_aw_valid_w, xbar_m04_aw_ready_w,
              xbar_m04_w_valid_w,  xbar_m04_w_ready_w,
              xbar_m04_b_valid_w,  xbar_m04_b_ready_w,
              xbar_m04_ar_valid_w, xbar_m04_ar_ready_w,
              xbar_m04_r_valid_w,  xbar_m04_r_ready_w,
              xbar_m04_r_last_w,   xbar_m04_w_last_w}),
    .probe1 (xbar_m04_aw_addr_w),
    .probe2 (xbar_m04_w_data_w),
    .probe3 (xbar_m04_ar_addr_w),
    .probe4 (xbar_m04_r_data_w),
    .probe5 ({xbar_m04_b_resp_w, xbar_m04_r_resp_w})
  );

  // ILA 10: AXI switch M05 ↔ RX2 status register map (AXI4-Lite; no last signals)
  ila_sw_sta2 i_ila_sw_sta2 (
    .clk    (core_clk_w),
    .probe0 ({xbar_m05_aw_valid_w, xbar_m05_aw_ready_w,
              xbar_m05_w_valid_w,  xbar_m05_w_ready_w,
              xbar_m05_b_valid_w,  xbar_m05_b_ready_w,
              xbar_m05_ar_valid_w, xbar_m05_ar_ready_w,
              xbar_m05_r_valid_w,  xbar_m05_r_ready_w,
              1'b0,                1'b0}),
    .probe1 (xbar_m05_aw_addr_w),
    .probe2 (xbar_m05_w_data_w),
    .probe3 (xbar_m05_ar_addr_w),
    .probe4 (xbar_m05_r_data_w),
    .probe5 ({xbar_m05_b_resp_w, xbar_m05_r_resp_w})
  );

  // ILA 11: TX2 FIFO ↔ BSG link TX2 core
  ila_tx_bsg2 i_ila_tx_bsg2 (
    .clk    (core_clk_w),
    .probe0 ({tx2_valid_w, tx2_ready_w}),
    .probe1 (tx2_data_w)
  );

  // ILA 12: RX2 FIFO ↔ BSG link RX2 core
  ila_rx_bsg2 i_ila_rx_bsg2 (
    .clk    (core_clk_w),
    .probe0 ({rx2_valid_w, rx2_yumi_w}),
    .probe1 (rx2_data_w)
  );

  // ILA 13: RX2 FIFO status signals ↔ RX2 status register map
  ila_rx_sta2 i_ila_rx_sta2 (
    .clk    (core_clk_w),
    .probe0 ({rx2_fifo_empty_w, rx2_fifo_full_w, rx2_fifo_count_w})
  );
`endif // SIM
  ...
  // =========================================================================
  // ===== Link 2 (reverse-direction link) — FIFOs, status, and PHY =====
  // Shares core_clk_w / core_clk90_w and the reset sequencer with link 1.
  // =========================================================================

  // ---- TX2 FIFO: AXI4 write slave → bsg_link TX2 core side ----
  logic                        tx2_valid_w;
  logic [FLIT_WIDTH_P-1:0]     tx2_data_w;
  logic                        tx2_ready_w;
  logic                        tx2_fifo_full_w;
  logic                        tx2_fifo_empty_w;
  logic [LG_TX_FIFO_DEPTH_P:0] tx2_fifo_count_w;
  ...
  bsg_link_axi_tx_fifo #(
    ...
    .clk_i          (core_clk_w),
    .rst_i          (rst_int_w),
    .axi_req_i      (tx2_axi_req_w),
    .axi_resp_o     (tx2_axi_resp_w),
    .tx_valid_o     (tx2_valid_w),
    .tx_data_o      (tx2_data_w),
    .tx_ready_i     (tx2_ready_w),
    .tx_fifo_full_o (tx2_fifo_full_w),
    .tx_fifo_empty_o(tx2_fifo_empty_w),
    .tx_fifo_count_o(tx2_fifo_count_w)
  );

  // ---- RX2 FIFO: bsg_link RX2 core side → AXI4 read slave ----
  logic                        rx2_valid_w;
  logic [FLIT_WIDTH_P-1:0]     rx2_data_w;
  logic                        rx2_yumi_w;
  logic                        rx2_fifo_full_w;
  logic                        rx2_fifo_empty_w;
  logic [LG_RX_FIFO_DEPTH_P:0] rx2_fifo_count_w;
  ...
  bsg_link_axi_rx_fifo #(
    ...
    .clk_i          (core_clk_w),
    .rst_i          (rst_int_w),
    .axi_req_i      (rx2_axi_req_w),
    .axi_resp_o     (rx2_axi_resp_w),
    .rx_valid_i     (rx2_valid_w),
    .rx_data_i      (rx2_data_w),
    .rx_yumi_o      (rx2_yumi_w),
    .rx_fifo_full_o (rx2_fifo_full_w),
    .rx_fifo_empty_o(rx2_fifo_empty_w),
    .rx_fifo_count_o(rx2_fifo_count_w)
  );

  // ---- RX2 Status registers ----
  logic rx2_fifo_flush_w;
  ...
  bsg_link_axi_rx_status #(
    ...
    .clk_i           (core_clk_w),
    .rst_i           (rst_int_w),
    .axi_req_i       (sta2_axi_req_w),
    .axi_resp_o      (sta2_axi_resp_w),
    .rx_fifo_empty_i (rx2_fifo_empty_w),
    .rx_fifo_full_i  (rx2_fifo_full_w),
    .rx_fifo_count_i (rx2_fifo_count_w),
    .rx_fifo_flush_o (rx2_fifo_flush_w)
  );

  // rx2_fifo_flush_w reserved for future use (same as link 1).

  // ---- bsg_link_ddr_upstream (TX2) — sends over the link-2 ribbon ----
  bsg_link_ddr_upstream #(
    .width_p                         (FLIT_WIDTH_P),
    .channel_width_p                 (CHANNEL_WIDTH_P),
    .num_channels_p                  (NUM_CHANNELS_P),
    ...
    .core_clk_i         (core_clk_w),
    .core_link_reset_i  (core_link_reset_int),
    .core_data_i        (tx2_data_w),
    .core_valid_i       (tx2_valid_w),
    .core_ready_o       (tx2_ready_w),
    .io_clk_i           (core_clk_w),
    .io_clk90_i         (core_clk90_w),
    .io_link_reset_i    (io_link_reset_int),
    .async_token_reset_i(async_token_reset_int),
    .io_clk_r_o         (upstream2_io_clk_r_o),
    .io_data_r_o        (upstream2_io_data_r_o),
    .io_valid_r_o       (upstream2_io_valid_r_o),
    .token_clk_i        (token_clk2_i)
  );

  // ---- bsg_link_ddr_downstream (RX2) — receives from the link-2 ribbon ----
  bsg_link_ddr_downstream #(
    .width_p                         (FLIT_WIDTH_P),
    .channel_width_p                 (CHANNEL_WIDTH_P),
    .num_channels_p                  (NUM_CHANNELS_P),
    ...
    .core_link_reset_i(core_link_reset_int),
    .io_link_reset_i  (downstream_io_link_reset_int),
    .core_data_o      (rx2_data_w),
    .core_valid_o     (rx2_valid_w),
    .core_yumi_i      (rx2_yumi_w),
    .io_clk_i         (downstream2_io_clk_i),
    .io_data_i        (downstream2_io_data_i),
    .io_valid_i       (downstream2_io_valid_i),
    .core_token_r_o   (downstream2_core_token_r_o)
  );
